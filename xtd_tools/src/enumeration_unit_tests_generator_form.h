  ...
  private:
    ...
    static std::vector<xtd::ustring> get_source_file_text(const xtd::ustring& enumeration, const xtd::ustring& namespace_str) {
      return {
        ...
        "    ustring result;",
        "    result += ustring::format(\"#include <{}/{}>\\n\", namespace_str.replace(\"::\", \"/\"), ustring::class_name<enum_t>());",
        "    result += \"#include <xtd/tunit/assert>\\n\";",
        "    result += \"#include <xtd/tunit/test_class_attribute>\\n\";",
        "    result += \"#include <xtd/tunit/test_method_attribute>\\n\";",
        "    result += \"\\n\";",
        "    result += ustring::format(\"using namespace {};\\n\", namespace_str);",
        "    result += \"using namespace xtd::tunit;\\n\";",
        "    result += \"\\n\";",
        "    result += ustring::format(\"namespace {}::tests {{\\n\", namespace_str);",
